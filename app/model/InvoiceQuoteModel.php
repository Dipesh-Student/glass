...
class InvoiceQuoteModel
{
    ...
    public function addNewInvoiceQuote($custid, $custname, $challanid, $inpdid, $ivpdname, $ivpddim, $ivpdquantity, $ivpdwork, $ivpdtdim, $ivpdrate, $ivtotal)
    {
        try {

            $this->connection->beginTransaction();

            $sql = "INSERT INTO `invoice_quote`(`customer_id`, `customer_name`,`iv_challan_id`,`iv_product_id`, `iv_product_name`, `iv_product_dim`, `iv_product_quantity`, `iv_product_work`, `iv_product_tdim`, `iv_product_rate`, `iv_total`) 
    ...
    public function getInvoiceQuoteByChallan($challanId)
    {
        try {
    ...
    {
        try {
            $sql = "SELECT * FROM `quotes` WHERE customer_id=:cid;";
            $stmt = $this->connection->prepare($sql);
    ...
    {
        try {
            $sql = "SELECT * FROM `invoice_quote` WHERE `iv_challan_id`=:challanid;";
            $stmt = $this->connection->prepare($sql);
            $stmt->bindParam(':challanid', $challanId);
            $stmt->execute();
            $totalRecord = $stmt->rowCount();
